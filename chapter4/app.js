(function () {
    function changeBudyColor() {
        return new Promise(function(resolve, reject){
            var bodyElement = document.getElementsByTagName('body')[0];
            bodyElement.className = 'soft-body';
            resolve(bodyElement);
        })
    }

    function addSubmitButtonToBody() {
        return new Promise(function(resolve, reject){
            if(bodyElement){
            var createdButton = document.createElement('button')
            createdButton.innerText = 'Submit';
            createdButton.id = 'submit-button';
            bodyElement.appendChild(createdButton);
            resolve(bodyElement);
            } else {
                reject('No doby available')
            }
        })
    }

    function waitUntilButtonIsClicked() {
        return new Promise(resolve, reject) {
            var button = document.getElementById('submit-button');
        // button.addEventListener("click", function(e){
        //     alert('Clicked! ')
        //     debugger;
        // });
        button.addEventListener('click', function(e){
        var helloWorldTitle = document.querySelector('h1');
        var successTitle = document.querySelector('h2');

        helloWorldTitle.className = 'hidden';
        successTitle.className = 'bolder';

        resolve('Success');
    });
        }
    }

    function showDelayedMessage(message) {
        return new Promise(resolve, reject) {
            setTimeout(function() {
                alert('Hey hey' + message)
            }, 500);
        }
    }

    changeBudyColor()
    .then(function(){
        return addSubmitButtonToBody(bodyElement);
    }).then((res) => {
        return waitUntilButtonIsClicked;
    })
    .then(() => {
        debugger;
    })
    .catch(e => {
        debugger;
    })


    

    // var multipleClickDiv = document.querySelector('.multiple-click');
    // multipleClickDiv.getEventListener('click', function(e){
    //     debugger;
    // });
})()